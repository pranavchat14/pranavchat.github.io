...
Let's dive into the world of Vision Language Models (VLMs) and their struggle with negative instructions.

Let's begin with some results to show that this actually does happen. Here are some of the results generated by the Stable Diffusion 3 model:
...
What should I do now? Should I go back and delete it? Of course, I can delete the elephant from the mental scene. But VLMs do not have the luxury to do that.

And maybe that gives a clue why VLMs struggle with negative instructions.
...
## The Elephant's Digital Cousin: How VLMs See the World

While training, VLMs are shown a vast amount of data. And through the course of training, they associate words with their visual counterparts on the basis of their co-occurrence. It creates sort of a loose mapping between words and features of objects they have seen.

What VLMs lack is the mapping of negation of each of those words and the appropriate corresponding visual representations. This limitation comes from the fact that during training, VLMs are primarily exposed to positive associations between words and images. They learn to generate images based on what should be present, rather than what should be absent.

The sequential nature of text processing in these models means that by the time a negative instruction is processed, the model may have already activated the visual features associated with the negated concept. Unlike us humans, who can consciously suppress and modify mental images, VLMs lack this flexible, top-down control over their generative process.
...
- Do you go about labeling all the images not containing the balloons as not-balloons?
